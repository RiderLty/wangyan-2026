<!-- 论文使用说明（转 Word 前删除本段）：
1. 全文约 1.1 万字，结构严格对应 docs/THESIS_OUTLINE.md，未增删小节。
2. 图的处理：第 3/4 章的架构图、ER 图、用例图、流程图由你手绘/画图工具绘制，文中已用“【待插图】”标注位置与画图要点；
   第 5 章截图均在 docs/assets/ 下（文件名即小节号），按标注粘贴即可。注意：5.3.5 相关截图为旧版式，建议按当前界面重截。
3. 附录 A/B/C 全文分别在 docs/appendix-ddl.sql、docs/appendix-compose.yml、docs/api.md，转 Word 时整篇粘贴到对应附录。
...
【待插图 图 4-3 系统E-R图。画图要点：以 notes 实体居中，users、teams 分列两侧；矩形为实体、菱形为联系；联系属性标注——团队成员表→role，邀请表→status/expires_at，share_links→permission/expires_at，recycle_bin→deleted_at/expires_at；folders 画出自引用联系（子文件夹）。】
...
笔记表是全库枢纽，如表 4-4 所示。team_id 为空表示个人笔记、非空表示团队笔记；folder_id 仅个人笔记使用（数据库 CHECK 约束保证团队笔记不挂个人文件夹）；content 以 JSONB 存储 ProseMirror 文档快照；content_text 为服务端派生的纯文本列，用于关键词检索；visibility 控制团队笔记的成员级可见性；deleted_at 为软删标记。围绕笔记表还有四张支撑表：folders（文件夹，自引用支持嵌套）、tags 与 note_tags（标签多对多）、yjs_updates（协作增量日志，二进制追加式存储，见 4.4.2）。
...
全库共 13 张基表，以 notes 为枢纽辐射展开，如图 4-4 所示；除正文各表外，还包括 folders、tags、note_tags、attachments、yjs_updates、team_invitations 六张支撑表，以及服务于只读统计的四个视图（活跃笔记、团队概览、回收站展示、最新版本号）。业务 CRUD 全部走基表，视图仅承载统计查询。

【待插图 图 4-4 数据库关系图。画图要点：以 notes 居中，按“一对多”连线画出其与 folders、note_versions、share_links、recycle_bin、note_tags、attachments、yjs_updates 的从属关系；users—teams 之间画“拥有”与“属于（经 team_members）”两条联系；各表旁标注主要字段与索引（如 notes(owner_id, deleted_at)、yjs_updates(note_id, id)）。】
...
文档同步采用“增量日志 + 快照”双轨设计，数据流如图 4-5 所示。打开文档时，服务端按自增序回放 yjs_updates 表中的历史增量；若该笔记从未有过协作会话，则从 notes.content 快照“播种”出初始 Yjs 文档并存入种子帧。协作过程中，每帧实时增量先在内存缓冲 2 秒、以 Y.mergeUpdates 合并为一帧后追加入库，避免逐键写行。最后一个连接断开时，将内存文档合并回写 notes.content 与 content_text，并压缩（compaction）已被合并的旧增量行。为防止异常会话破坏数据，回写前执行防误清检查：若会话文档为空而库中快照非空，则跳过回写。增量广播走内存路径，与持久化完全异步，保证键入传播不受落盘耗时影响。

【待插图 图 4-5 Yjs文档同步流程图。画图要点：泳道图（客户端 A / 客户端 B / 服务端）；打开文档→回放增量或播种→实时编辑→增量广播→2 秒缓冲合并入库→末连接断开→合并回写快照+压缩；标注“防误清检查”分支。】
...
系统接口按资源分组设计，全部遵循 4.6 节统一的请求/响应约定，完整接口清单见附录 C，此处列出各组的代表性接口。
...
**图 5-1 项目目录结构**（注：此处以目录树呈现，如需截图可补充 IDE 目录视图）

### 5.1.3 Docker开发环境搭建

数据层容器常驻 NAS：通过两条 docker run 命令分别部署 PostgreSQL 16 与 Redis 7 容器，数据目录统一挂载在 NAS 的项目数据目录下，备份与迁移只需拷贝该目录。高位端口（15432/16379）是为避开 NAS 上已被占用的默认端口。部署脚本存档于仓库，容器重建即完全恢复，开发环境的搭建与销毁成本极低。
...
【插图 docs/assets/5.2.1-register.png —— 图 5-2 用户注册界面】
...
【插图 docs/assets/5.2.2-login.png —— 图 5-3 用户登录界面】
...
【插图 docs/assets/5.2.4-home.png —— 图 5-4 登录后工作台首页】

## 5.3 个人笔记管理模块实现

### 5.3.1 笔记的创建与编辑

笔记编辑采用 Tiptap 富文本编辑器，文档模型为 ProseMirror JSON，与后端 JSONB 存储格式一致，前后端之间无需任何格式转换。编辑过程中前端以 800 毫秒防抖调用 `PATCH /notes/:id` 自动保存，状态栏以“已保存 / 有未保存修改 / 保存中 / 保存失败”四态反馈保存进展；空标题不提交，服务端兜底为“未命名笔记”。删除为软删除：置 deleted_at 并写入回收站元数据（30 天后到期），列表查询一律过滤已删标记；对他人笔记的访问一律返回 404，实现越权防护。编辑界面如图 5-5 所示。

【插图 docs/assets/5.3.1-note-edit.png —— 图 5-5 笔记编辑界面】

### 5.3.2 Markdown实时预览

实时预览采用 Tiptap 的 input rules 机制实现“所写即所见”：输入 `# `、`- `、`> ` 等语法前缀时，编辑器即时将段落转换为对应节点（标题、列表、引用、代码块等），无需左源码右预览的双栏布局——这与 Tiptap“编辑视图即渲染视图”的架构选型一致，也为 5.4 的实时协作奠定了基础（协作只在单一文档模型上才成立）。初版实现了标题、列表、引用、代码块、加粗斜体等核心语法；随后迭代补充了图片、表格、链接与任务列表的语法输入与渲染，并引入 GitHub 风格的白色渲染主题，使输出观感与主流技术社区一致，如图 5-6 所示。

【插图 docs/assets/5.3.2-realtime-preview.png —— 图 5-6 Markdown 语法即时渲染】
...
【插图 docs/assets/5.3.3-folders.png —— 图 5-7 文件夹与标签管理】

### 5.3.4 笔记搜索功能

搜索覆盖标题与正文两个字段。正文检索基于保存时派生的 content_text 纯文本列，以 PostgreSQL 的 ILIKE 子串匹配实现，输入关键词中的 `%` 与 `_` 通配符做了转义处理，防止其被解释为匹配模式。选择 ILIKE 而非 tsvector 全文索引的原因是：PostgreSQL 内置分词器不切分中文，对中文子串检索基本无效，在当前数据规模下子串匹配已足够且实现简单；中文分词检索（zhparser）列入改进方向。搜索词支持经 URL 参数深链，便于分享检索结果，如图 5-8 所示。

【插图 docs/assets/5.3.4-search.png —— 图 5-8 笔记搜索】

### 5.3.5 界面展示与核心代码

工作台采用“文件夹树（左）+ 笔记列表（中）+ 编辑器（右）”三栏布局，编辑区宽度自适应，支持专注模式（编辑器独占整页、Esc 退出）。本模块论文可引用的核心代码包括：notes.service.ts（CRUD/搜索/越权防护）、prosemirror.util.ts（JSONB 转纯文本派生列）、NoteEditorPanel.tsx（Tiptap 集成）、HomePage.tsx（三栏工作台与防抖自动保存）。

【插图 docs/assets/5.3.1-note-edit.png 或重截 —— 图 5-9 工作台整体布局（建议按当前版式重截）】

## 5.4 实时协作编辑模块实现

### 5.4.1 WebSocket服务端实现

服务端复用 y-websocket 的 setupWSConnection 参考实现，将其挂载在 NestJS HTTP server 的 upgrade 事件上，与 REST 接口共进程、共端口（路径 `/ws/:noteId`）。握手期完成双重校验：JWT 有效性（复用认证模块的 JwtModule）与笔记访问权限（复用权限矩阵），失败即 401 拒绝连接。持久化通过实现 y-websocket 的 IDatabasePersistence 接口接入：bindState 负责打开文档时的回放/播种，writeState 负责末连接断开时的合并回写。实现中发现并修复了一个播种竞态：y-websocket 不等待 bindState 完成，“连接即断→销毁→再连接”会让同一快照被播种两次、内容出现两份，修复方案是按笔记加异步互斥锁，将 bindState/writeState 串行化。

### 5.4.2 Yjs前端集成

前端在笔记编辑面板中将 Tiptap 的 Collaboration 扩展与 WebsocketProvider 绑定：编辑器的每次按键经 y-prosemirror 转化为 Yjs 增量，由 Provider 推送到服务端并接收他人增量，编辑界面如图 5-10 所示。文档内容的真实来源自此切换为 Yjs 文档，服务端在会话结束时将最终状态合并回写 notes.content。会话生命周期（Y.Doc 与 Provider 的创建、销毁）由 useEffect 严格管理——开发期曾因 useMemo 在 React StrictMode 下的双调用泄漏连接，产生“幽灵在线用户”，据此加固。服务端还有一道防误清保护：同步未完成即断开的空会话不会覆盖库中非空快照。

【插图 docs/assets/5.4.2-yjs-integration.png —— 图 5-10 Yjs 驱动的协作编辑器（连接状态与在线人数）】

### 5.4.3 多用户光标同步

光标同步由 Tiptap CollaborationCursor 扩展与 awareness 协议实现：每个客户端广播本地用户名、随机分配的显示颜色与光标相对位置；其他客户端将其渲染为带姓名标签的彩色光标，状态栏实时显示在线人数，如图 5-11 所示。

【插图 docs/assets/5.4.3-cursor-sync.png —— 图 5-11 双窗口多光标同步效果】

### 5.4.4 冲突合并与一致性保证

为验证 CRDT 收敛性质，编写了 Node 双客户端并发编辑脚本：两个客户端先后连接同一笔记，A 在文首插入段落、B 在文末插入段落，双方互不知晓对方操作；持续同步后断言两端文档内容逐字节一致（A===B），实验结果与预期一致，如图 5-12 所示。在此基础上，服务端会话收尾时回写的快照已包含双方的并发编辑。

【插图 docs/assets/5.4.4-crdt-merge.png —— 图 5-12 并发编辑合并结果】
...
团队模块实现团队的创建、信息维护、解散与成员管理。创建团队时创建者自动写入 team_members 表（role=owner），与 teams.owner_id 双写一致；解散团队前校验团队内仍有笔记则拒绝，防止误删。成员管理支持角色调整（仅 owner 可操作）、移除成员（owner/admin/本人可发起，创建者不可被移除）与主动退队，管理界面如图 5-13 所示。

【插图 docs/assets/5.5.1-team-manage.png —— 图 5-13 团队管理界面】

### 5.5.2 团队成员邀请与审批

邀请流程实现为：owner/admin 按邮箱发起邀请（有效期 7 天），被邀请人在“我的邀请”列表中接受或拒绝，邀请人可撤回，过期邀请由定时任务置为 expired。同一团队对同一邮箱只允许一条有效邀请（数据库唯一约束）；对已存在但非 pending 的旧邀请记录做复用（重置状态）而非新增。接受邀请即插入成员记录（role=member）并置邀请状态为 accepted。邀请列表与审批界面如图 5-14 所示。

【插图 docs/assets/5.5.2-invitation.png —— 图 5-14 邀请与审批界面】

### 5.5.3 团队笔记权限分配

团队笔记按四级访问矩阵实现权限判定（owner / team_admin / team_edit / team_read），判定集中在 notes.service.ts 的 getAccessLevel 函数与 teams.service.ts 的角色断言函数中。成员查看团队笔记列表时按角色过滤：owner/admin 可见全部，普通成员可见非私有及自己创建的笔记。WebSocket 协作连接的权限与编辑权限对齐：只读成员的连接请求被 401 拒绝（见 4.5.4），从传输层防止只读写穿。团队笔记可见性设置界面如图 5-15 所示。

【插图 docs/assets/5.5.3-team-note-visibility.png —— 图 5-15 团队笔记可见性设置】
【插图 docs/assets/5.5.4-team-readonly.png —— 图 5-16 只读成员视角】

### 5.5.4 界面展示与核心代码

本模块论文可引用的核心代码包括：teams.service.ts（RBAC 集中实现、邀请生命周期、可见性过滤）、realtime.service.ts（WS 权限对齐）、TeamManageModal.tsx（团队管理弹窗）。

## 5.6 笔记分享模块实现

### 5.6.1 分享链接生成

分享设置弹窗支持选择权限（只读/可编辑）与有效期（永久或指定时间），如图 5-17 所示。服务端以 Node crypto 的 randomBytes 生成 16 字节安全随机数、编码为 32 位十六进制 token，写入 share_links 表；创建与变更分享仅限笔记所有者或团队 owner/admin。

【插图 docs/assets/5.6.1-share-modal.png —— 图 5-17 分享链接生成界面】

### 5.6.2 链接访问权限控制

访客公开页 `/s/:token` 无需登录：先经公开解析接口获取链接元数据（该接口对停用/过期/已删/伪造统一返回 404 防枚举），再拉取正文快照渲染；解析结果经 Redis 缓存 60 秒，管理端变更即时失效缓存，访问计数则实时累加不缓存。持可编辑链接的访客可凭 token 通过 WebSocket 握手鉴权进入同一 Yjs 文档，与登录用户实时协作，效果如图 5-18 所示——访客协作复用了 5.4 的整条链路，仅鉴权方式不同（JWT 校验失败时回落到分享 token 校验）。

【插图 docs/assets/5.6.2-guest-collab.png —— 图 5-18 访客实时协作】
...
版本快照实现“手动 + 关键事件”策略：用户可手动保存版本；编辑会话结束且有变更时，服务端在 writeState 收尾中自动建版（source=auto）；回滚前当前状态自动留档（source=rollback）。自动建版以回写前后的纯文本是否有变化为触发条件，且与最新版本做“标题+内容”双比较去重（首测曾只比较内容，纯改标题场景会丢失回滚前快照，已修复）。版本列表展示版本号、标题、来源与时间，抽屉式界面如图 5-19 所示。

【插图 docs/assets/5.7.1-version-drawer.png —— 图 5-19 版本历史抽屉】
...
软删除的笔记进入回收站，列表展示标题、删除时间与剩余保留天数（由视图计算），如图 5-20 所示。恢复操作清除软删标记并将笔记放回原文件夹（原文件夹已删除则保持未归档）；彻底删除由 notes 行的外键级联一并清除版本、分享链接、标签关联与协作增量，无需手工清理链。

【插图 docs/assets/5.7.3-recycle-bin.png —— 图 5-20 回收站界面】
...
导出入口位于编辑器工具栏的“导出”菜单，如图 5-21 所示。本模块论文可引用的核心代码包括：utils/export.ts（Markdown 序列化、下载与打印视图入口）、PrintPage.tsx（PDF 打印排版）。

【插图 docs/assets/5.8.3-export-menu.png —— 图 5-21 导出菜单】
...
对代表性接口经 Nginx 反代、局域网访问采样（登录 20 次，其余各 50 次），结果如表 6-9 所示。
...
本文围绕“个人与团队的知识记录与协作”需求，完成了在线 Markdown 笔记编辑与管理平台的设计与实现。主要工作包括：（1）完成了系统需求分析，划分四类用户角色与七个功能模块，明确功能与非功能需求；（2）完成了系统设计，包括前后端分离的四层架构、13 张数据表的数据模型、“增量日志 + 快照”双轨的实时协作机制与四级权限矩阵；（3）基于 React 18 + TypeScript + Ant Design、NestJS、PostgreSQL（JSONB）、Redis、Yjs + WebSocket、JWT 完成了系统实现，覆盖全部规划模块与 Markdown/PDF 导出、容器化部署；（4）以 121 条功能用例（100% 通过）与三类性能测试验证了系统：接口平均响应 5.3ms、实时编辑传播延迟平均 0.8ms、200 路并发协作连接全部成功。系统最终部署于 NAS 设备并完成端到端验证，达到可现场演示的状态。
...
核心数据表（含 4 个视图）的完整建表语句见仓库文件 `docs/appendix-ddl.sql`（671 行），转 Word 时将该文件全文粘贴至此。以下摘录笔记表以示例：
...
完整编排文件见仓库文件 `docs/appendix-compose.yml`，转 Word 时全文粘贴至此。该文件编排 postgres、redis、server、web 四个服务，含健康检查、依赖顺序与重启策略。

## 附录C：核心接口API文档

完整接口文档见仓库文件 `docs/api.md`（认证、笔记、团队、分享、版本与回收站、WebSocket 事件六组），转 Word 时全文粘贴至此。
