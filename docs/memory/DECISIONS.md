...
## D-011：版本快照与回滚实现策略（5.7）

- 日期：2026-09-10
- 背景：D-007 已定快照策略（手动+关键事件，不做纯定时）；回滚如何与 Yjs 实时协作共存需决策。
- 选项（回滚落地）：① 直接 PATCH notes.content（会被协作增量回放覆盖，且在线客户端不可见）；② 通过 y-websocket 内存文档做 CRDT 回滚（清空 fragment 按快照重建，transact 原子提交）
- 决定：**双路径**——文档在内存（有协作会话）走 ②，在线客户端实时看到回滚、增量自然入日志；冷文档走 DB 回写 + 作废旧增量帧（下次播种以快照为准）。理由：②保证回滚不与 CRDT 收敛冲突，①在协作场景必被覆盖。
- 要点：
  1. 回滚前当前状态自动存版本（source=rollback）；与最新版本"标题+内容"均相同才跳过（首测只比内容，纯改标题场景丢失回滚前快照，已修复并记录 VER-05）
  2. 会话结束自动建版（source=auto）在 writeState 内实现，content_text 变化即触发，去重逻辑同上
  3. 回收站恢复放回 original_folder_id（原文件夹已删则保持未归档）；彻底删除靠 notes 行外键级联，不写手工清理链
